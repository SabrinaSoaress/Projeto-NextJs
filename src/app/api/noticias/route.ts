...
export async function PUT(req: Request) {
  try {
    const { id, dadosAtualizados } = await req.json(); // O id agora vem no corpo, junto com os dados
    const noticias = readNoticias(); // Lê as notícias do arquivo
    ...
    writeNoticias(noticias); // Salva as notícias de volta no arquivo
